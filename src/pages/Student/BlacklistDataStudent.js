...
import '../CSS/BlacklistDataStudent.css'
...
const BlacklistData = ({ classroomId, studentId }) => {
  ...
  const handleToggleGauge = () => {
    setShowGauge((prev) => {
      if (!prev) setShowBlacklist(false); // Hide blacklist if showing gauge
      return !prev;
    });
  };
  ...
  const handleToggleBlacklist = () => {
    setShowBlacklist((prev) => {
      if (!prev) setShowGauge(false); // Hide gauge if showing blacklist
      return !prev;
    });
  };
...
const gaugeOptions = {
  ...
  <button className="filter-button" onClick={handleToggleGauge}>
    {showGauge ? "Hide Gauge" : "Show Gauge"}
  </button>

  <button className="filter-button" onClick={handleToggleBlacklist}>
    {showBlacklist ? "Hide Blacklist Data" : "Show Blacklist Data"}
      ...
      <div style={{ display: "flex", flexDirection: "row", alignItems: "flex-start" }}>
        {/* Conditional Gauge Chart Rendering */}
        {showGauge && (
          <ReactECharts option={gaugeOptions} style={{ height: "400px", width: "300px", marginRight: "20px" }} />
        )}

        {/* Conditional Blacklist Table Rendering */}
        {showBlacklist && blacklistData.length > 0 && (
          <div style={{ flex: 1 }}>
