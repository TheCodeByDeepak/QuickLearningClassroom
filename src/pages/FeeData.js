...
const FeeData = ({ classroomId, studentId }) => {
  ...
  const handleDeleteFee = async (receiptNo, classroomId, studentId, date) => {
    try {
      // Send the necessary data in the body of the DELETE request
      await axios.delete(
        `http://localhost:5000/api/fees/deleteFee`,
        {
          data: { receiptNo, classroomId, studentId, date }, // Include data here
        }
      );
  
      // Update state to remove the deleted record
      setFeeData((prev) =>
        prev.filter((fee) => fee[0] !== receiptNo) // Filter out deleted fee by receiptNo
      );
      setFilteredFeeData((prev) =>
        prev.filter((fee) => fee[0] !== receiptNo) // Filter out deleted fee by receiptNo
      );
      ...
      setError("Failed to delete fee record."); // Set error message
    }
  };
  ...
  return (
    ...
    {successMessage && (
      ...
      {/* Date Filters */}
      <div className="filter-container-fee-admin">
        <label htmlFor="startDate" className="filter-label-fee-admin">Start Date: </label>
        <input
          type="date"
          id="startDate"
          value={startDate}
          onChange={handleStartDateChange}
           className="filter-input-fee-admin start-date-fee-admin"
        />
        <label htmlFor="endDate" className="filter-label-fee-admin">End Date: </label>
        <input
          type="date"
          id="endDate"
          value={endDate}
          onChange={handleEndDateChange}
          className="filter-input-fee-admin end-date-fee-admin"
        />
        <button className="filter-button-fee-admin" onClick={handleApplyFilter}>Apply Filter</button>
        <button className="filter-button-fee-admin" onClick={handleClearFilter}>Clear Filter</button>

        {/* Toggle Button for Showing/Hiding the Graph */}
        <button className="filter-button-fee-admin" onClick={toggleGraphVisibility}>
          {showGraph ? "Hide Graph" : "Show Graph"}
        </button>

        {/* Toggle Button for Showing/Hiding the Fee Data Table */}
        <button className="filter-button-fee-admin" onClick={toggleTableVisibility}>
      ...
      {showTable && filteredFeeData.length > 0 && (
        <table className="fee-table-fee-admin">
          <thead>
            <tr>
              <th>Receipt No</th>
              <th>Name</th>
          ...
          <tbody>
            {filteredFeeData.map((row, index) => (
              <tr key={index}>
                ...
                <td>
                  <button
                    onClick={() => handleDeleteFee(row[0], classroomId, row[9], row[3])}
                 style={{
                        color: 'red',
                        fontSize: '20px',
                        border: 'none',
