...
const TestMarksData = ({ classroomId, studentId }) => {
  ...
  const [message, setMessage] = useState(''); // State to show delete message
  ...
  const handleDelete = async (classroomId, studentId, date, subject) => {
   
  
    setFilteredData((prevData) =>
      prevData.filter((record) => {
        // Convert record's date to a proper Date format
        const recordDate = new Date(record.date).toISOString().slice(0, 10); // Format the date (e.g., "2024-12-27")
        const requestDate = new Date(date).toISOString().slice(0, 10); // Format the requested date
        // Compare the dates only, ignore other fields for now
        return recordDate !== requestDate;
      })
    );
  
    try {
      const response = await axios.delete('http://localhost:5000/api/testmarks/deleteTestMarks', {
        data: { classroomId, studentId, date, subject }
      });
      ...
      console.error('Error deleting test marks:', err);
      setError('Failed to delete the record');
      //fetchTestData(); // Refetch data to restore deleted record if deletion fails
    }
  };
